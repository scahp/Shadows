...
void jGame::ProcessInput()
{
	...
	{
		//MainCamera->MoveForward(speed);
		if (FontPrimitive)
		{
			FontPrimitive->Size.x += 10;
			FontPrimitive->Size.y = FontPrimitive->Size.x;
		}
	}
	if (g_KeyState['s'] || g_KeyState['S'])
	{
		...
		{
			FontPrimitive->Size.x -= 10;
			FontPrimitive->Size.x = Max<float>(FontPrimitive->Size.x, 10);
			FontPrimitive->Size.y = FontPrimitive->Size.x;
		}
	}
...
void jGame::Setup()
{
	...
	DirectionalLightShadowMapUIDebug = jPrimitiveUtil::CreateUIQuad({ 0.0f, 0.0f }, { 150, 150 }, false, DirectionalLight->GetShadowMap());
