...
void jGame::Update(float deltaTime)
{
	...
	if (!IsInitialized)
	{
		...
		Radiosity::InputParams::InitRadiosity(RadiosityParams);
	...
	DisplayResults(RadiosityParams, FoundShootPatch);		// 4
	...
	if (FoundShootPatch == -1)
		TitleText = " Converged : " + std::to_string(TotalProcessingCounts);
	else
